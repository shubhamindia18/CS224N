...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding") as scope:
        L = tf.get_variable("embedding_table",[len(self.vocab), self.config.embed_size],
                                    initializer=tf.random_uniform_initializer(-1,1), trainable=True)
        for i in np.arange(self.config.num_steps):
          scope.reuse_variables()
          x = tf.nn.embedding_lookup(L, self.input_placeholder[:,i])
          x = tf.reshape(x, shape= (self.config.batch_size, self.config.embed_size))
          inputs.append(x)
      ### END YOUR CODE
    # with tf.device('/cpu:0'):
    # ### YOUR CODE HERE
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection_layer'):
      U = tf.get_variable('weights', shape=(self.config.hidden_size, len(self.vocab)),initializer=xavier_weight_init())
      b_2 = tf.get_variable('bias', shape = (1, len(self.vocab)), initializer=tf.zeros_initializer())
    for h in rnn_outputs:
      output = tf.matmul(h,U) + b_2
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    # output = tf.reshape(output, shape=(-1, self.config.num_steps, len(self.vocab)))
    all_ones = [tf.ones([self.config.batch_size * self.config.num_steps])]

    # loss = sequence_loss(output, tf.reshape(self.labels_placeholder, [-1]), tf.ones(shape=(self.config.batch_size, self.config.num_steps)))
    with tf.variable_scope('loss'):
      loss = sequence_loss([output], [tf.reshape(self.labels_placeholder, [-1])], all_ones)
