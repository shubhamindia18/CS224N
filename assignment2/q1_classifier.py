...
class SoftmaxModel(Model):
  ...
  def create_feed_dict(self, input_batch, label_batch):
    ...
    feed_dict[self.label_placeholder] = label_batch if label_batch is not None else None
  ...
  def add_model(self, input_data):
    ...
    # n_classes = tf.get_shape(self.label_placeholder)[1]
    # n_features = tf.get_shape(self.input_placeholder)[1]
    # n_classes = self.label_placeholder.get_shape()[1]
    # n_features = self.input_placeholder.get_shape()[1]
    n_classes = self.config.n_classes
    n_features = self.config.n_features
    with tf.name_scope("softmax"):
      W = tf.get_variable('weights',(n_features, n_classes), initializer = tf.constant_initializer(0.0))
      b = tf.get_variable('biases', (1, n_classes), initializer = tf.constant_initializer(0.0))
...
def test_SoftmaxModel():
  ...
  with tf.Graph().as_default():
    ...
    # the computation graph is being made for this object under the global computational graph
    ...
    # the above run was used to initialise the variables and now another sess.run will be called within
    #  run_epoch to actually run the computation
